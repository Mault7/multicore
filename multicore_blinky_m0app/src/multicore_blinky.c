...
void M4_IRQHandler(void)
{
	...
	case PID_BLINKY:
		while (msg.data0) {
			...
			Chip_GPIO_SetPinState(LPC_GPIO_PORT, 1, 11, msg.data1 & 1);
			Chip_GPIO_SetPinState(LPC_GPIO_PORT, 1, 12, msg.data1 & 1);
			Chip_GPIO_SetPinState(LPC_GPIO_PORT, 0, 14, !msg.data1);
